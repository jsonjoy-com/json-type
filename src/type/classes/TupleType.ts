...
> {
  ...
  public codegenValidator(
    ...
    ctx.js(
      /* js */ `if (!Array.isArray(${r}) || ${r}.length !== ${types.length}) return ${err};`,
    );
  ...
  public codegenJsonTextEncoder(
    ...
    for (let i = 0; i < last; i++) {
      types[i].codegenJsonTextEncoder(
        ctx,
        new JsExpression(() => `${value.use()}[${i}]`),
      );
      ctx.writeText(",");
    }
    types[last].codegenJsonTextEncoder(
      ctx,
      new JsExpression(() => `${value.use()}[${last}]`),
    );
    ctx.writeText("]");
  }
  ...
  private codegenBinaryEncoder(
    ...
    for (let i = 0; i < length; i++)
      if (ctx instanceof CborEncoderCodegenContext)
        ...
        types[i].codegenMessagePackEncoder(
          ctx,
          new JsExpression(() => `${r}[${i}]`),
        );
  }
  ...
  public toJson(
    ...
    for (let i = 0; i < last; i++)
      str +=
        (types[i] as any).toJson((value as unknown[])[i] as any, system) + ",";
    str += (types[last] as any).toJson(
      (value as unknown[])[last] as any,
      system,
    );
