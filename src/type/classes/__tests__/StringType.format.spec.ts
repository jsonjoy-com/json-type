...
describe("StringType format validation", () => {
  ...
  describe("UTF-8 format", () => {
    ...
    test("rejects strings with unpaired surrogates", () => {
      ...
      expect(validator(highSurrogate + lowSurrogate + highSurrogate)).toBe(
        true,
      ); // Unpaired at end
    });
  ...
  describe("Schema validation", () => {
    test("validates format values", () => {
      expect(() =>
        t.String({ format: "ascii" }).validateSchema(),
      ).not.toThrow();
      expect(() => t.String({ format: "utf8" }).validateSchema()).not.toThrow();
      expect(() =>
        t.String({ format: "invalid" as any }).validateSchema(),
      ).toThrow("INVALID_STRING_FORMAT");
    });

    test("validates format and ascii consistency", () => {
      expect(() =>
        t.String({ format: "ascii", ascii: false }).validateSchema(),
      ).toThrow("FORMAT_ASCII_MISMATCH");
      expect(() =>
        t.String({ format: "ascii", ascii: true }).validateSchema(),
      ).not.toThrow();
      expect(() =>
        t.String({ format: "utf8", ascii: true }).validateSchema(),
      ).not.toThrow(); // UTF-8 can have ascii=true
    });
  });
