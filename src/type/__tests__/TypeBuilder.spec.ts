...
test("array of any with options", () => {
  const type = t
    .Array(t.any.options({ description: "Any type" }))
    .options({ intro: "An array of any type" });
  expect(type.getSchema()).toStrictEqual({
    kind: "arr",
...
describe("validateSchema()", () => {
  test("can validate a number schema", () => {
    ...
    expect(() =>
      t.import({ ...schema, description: 123 } as any).validateSchema(),
    ).toThrow(new Error("INVALID_DESCRIPTION"));
    expect(() =>
      t.import({ ...schema, title: 123 } as any).validateSchema(),
    ).toThrow(new Error("INVALID_TITLE"));
    expect(() =>
      t.import({ ...schema, intro: null } as any).validateSchema(),
    ).toThrow(new Error("INVALID_INTRO"));
    expect(() =>
      t.import({ ...schema, gt: null } as any).validateSchema(),
    ).toThrow(new Error("GT_TYPE"));
    expect(() =>
      t.import({ ...schema, lt: null } as any).validateSchema(),
    ).toThrow(new Error("LT_TYPE"));
    expect(() =>
      t.import({ ...schema, gte: "334" } as any).validateSchema(),
    ).toThrow(new Error("GTE_TYPE"));
    expect(() =>
      t.import({ ...schema, lte: "334" } as any).validateSchema(),
    ).toThrow(new Error("LTE_TYPE"));
    expect(() =>
      t.import({ ...schema, lt: 1, gt: 2 } as any).validateSchema(),
    ).toThrow(new Error("GT_LT"));
    expect(() =>
      t.import({ ...schema, format: "int" } as any).validateSchema(),
    ).toThrow(new Error("FORMAT_INVALID"));
    expect(() =>
      t.import({ ...schema, validator: 123 } as any).validateSchema(),
    ).toThrow(new Error("INVALID_VALIDATOR"));
  });
  ...
  test("can validate a string schema", () => {
    ...
    expect(() =>
      t.import({ ...schema, min: null } as any).validateSchema(),
    ).toThrow(new Error("MIN_TYPE"));
    expect(() =>
      t.import({ ...schema, max: "asdf" } as any).validateSchema(),
    ).toThrow(new Error("MAX_TYPE"));
    expect(() =>
      t.import({ ...schema, min: -1 } as any).validateSchema(),
    ).toThrow(new Error("MIN_NEGATIVE"));
    expect(() =>
      t.import({ ...schema, max: -1 } as any).validateSchema(),
    ).toThrow(new Error("MAX_NEGATIVE"));
    expect(() =>
      t.import({ ...schema, max: 0.5 } as any).validateSchema(),
    ).toThrow(new Error("MAX_DECIMAL"));
    expect(() =>
      t.import({ ...schema, min: 1.2 } as any).validateSchema(),
    ).toThrow(new Error("MIN_DECIMAL"));
    expect(() =>
      t.import({ ...schema, min: 5, max: 3 } as any).validateSchema(),
    ).toThrow(new Error("MIN_MAX"));
    expect(() =>
      t.import({ ...schema, ascii: 123 } as any).validateSchema(),
    ).toThrow(new Error("ASCII"));
    expect(() =>
      t.import({ ...schema, ascii: "bytes" } as any).validateSchema(),
    ).toThrow(new Error("ASCII"));
  });
  ...
  test("validates object", () => {
    ...
    expect(() => type.validateSchema()).toThrow(
      new Error("UNKNOWN_FIELDS_TYPE"),
    );
  });
