...
describe("OrType", () => {
  ...
  test("can automatically infer discriminator in objects", () => {
    ...
    expect(() =>
      or.validate({ op: "replace2", path: "/foo", value: 123 }),
    ).toThrow();
    expect(() =>
      or.validate({ op: "add", path: 123, value: { foo: "bar" } }),
    ).toThrow();
    ...
    expect(() =>
      or.validate({ op: "copy", path: "/abc", fromd: "/xyz" }),
    ).toThrow();
    expect(() =>
      or.validate({ op: "remove", path: "/abc", from: "/sdf" }),
    ).toThrow();
    expect(() => or.validate([])).toThrow();
    expect(() => or.validate({})).toThrow();
