...
test("can export recursive schema", () => {
  ...
  const { t } = system;
  const post = system.alias(
    "Post",
    t.Object(t.prop("id", t.str), t.propOpt("author", t.Ref("User"))),
  );
  const stream = system.alias(
    "Stream",
    t.Object(t.prop("id", t.str), t.prop("posts", t.Array(t.Ref("Post")))),
  );
