...
test("add circular reference test", () => {
  ...
  const user = system.alias(
    "User",
    t.Object(t.prop("id", t.str), t.propOpt("address", t.Ref("Address"))),
  );
  const address = system.alias(
    "Address",
    t.Object(t.prop("id", t.str), t.propOpt("user", t.Ref("User"))),
  );
  const value1 = {
    id: "user-1",
    address: {
