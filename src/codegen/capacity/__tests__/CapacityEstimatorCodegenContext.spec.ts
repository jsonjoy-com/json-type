...
describe("binary", () => {
  test("empty", () => {
    ...
    const estimator = type.compileCapacityEstimator({});
    expect(estimator(new Uint8Array())).toBe(
      maxEncodingCapacity(new Uint8Array()),
    );
  });
  ...
  test("small buffer", () => {
    ...
    const type = system.t.bin;
    const estimator = type.compileCapacityEstimator({});
    expect(estimator(new Uint8Array([1, 2, 3]))).toBe(
      maxEncodingCapacity(new Uint8Array([1, 2, 3])),
    );
  });
});
...
describe("array", () => {
  ...
  test("simple elements", () => {
    ...
    const type = system.t.arr;
    const estimator = type.compileCapacityEstimator({});
    expect(estimator([1, true, "asdf"])).toBe(
      maxEncodingCapacity([1, true, "asdf"]),
    );
  });
...
describe("object", () => {
  ...
  test("one required and one optional keys", () => {
    ...
    const type = system.t.Object(
      system.t.prop("abc", system.t.str),
      system.t.propOpt("key", system.t.num),
    );
    const estimator = type.compileCapacityEstimator({});
    expect(estimator({ abc: "foo", key: 111 })).toBe(
      maxEncodingCapacity({ abc: "foo", key: 111 }),
    );
  });
});
...
describe("ref", () => {
  test("two hops", () => {
    ...
    system.alias(
      "User",
      system.t.Object(
        system.t.prop("id", system.t.Ref("Id")),
