...
const any = (ctx: CborEncoderCodegenContext, value: JsExpression, type: Type): void => {
...
const bool = (ctx: CborEncoderCodegenContext, value: JsExpression): void => {
  const v = value.use();
...
const bin = (ctx: CborEncoderCodegenContext, value: JsExpression, type: Type): void => {
  const v = value.use();
  ctx.js(/* js */ `encoder.writeBin(${v});`);
};

const const_ = (ctx: CborEncoderCodegenContext, value: JsExpression, type: Type): void => {
