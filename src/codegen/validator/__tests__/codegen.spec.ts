...
const exec = (
  ...
  json: unknown,
  error: any,
  options: Partial<ValidatorCodegenContextOptions> = {},
) => {
  ...
  expect(result2).toStrictEqual(
    !error ? "" : JSON.stringify([error.code, ...error.path]),
  );
...
test("validates according to schema a POJO object", () => {
  ...
  const json = {
    collection: {
      ...
      src: '{"foo": "bar"}',
      authz:
        'export const (ctx) => ctx.userId === "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx";',
      tags: ["foo", "bar"],
    },
...
describe("custom validators", () => {
  ...
  test("throws if custom validator is not provided", () => {
    ...
    expect(() => type.compileValidator({ errors: "object" })).toThrow(
      new Error("Validator [name = assetId] not found."),
    );
  });
